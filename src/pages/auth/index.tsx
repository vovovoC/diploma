...
import { LoginContent, RegisterContent, ResetPasswordContent } from "../../features/auth";
import { Notification } from "../../app/components/Notification";

type ntfType = { id: string, type: string, title: string, msg: string };

export const AuthPage = ({ type }: { type: string }) => {
  const [showFront, setShowFront] = useState<any>({login: true, register: false, reset: false});
  const [ntfList, setNtfList] = useState<ntfType[]>([]);

  useEffect(() => {
    setShowFront({login:  type == "login",  register: type == "register", reset: type == "resetPsw"});
  }, [type]);
    ...
    <div className="register">
      ...
      <div className="flippable-card-container">
        <CSSTransition in={showFront.login} timeout={500} classNames="flip">
          <div className="card">
            <div className={showFront.register ? "card-back": "card-back notshow"} >
              <RegisterContent />
            </div>
            <div className={showFront.login ? "card-front": "card-front  notshow"}>
              <LoginContent ntfList={ntfList} showNtf={showNtf}/>
            </div>
            <div className={showFront.reset ? "card-back": "card-back notshow"}>
